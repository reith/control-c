define(['jquery', 'i18n!nls/dict'], function($, Dict) {
	var TableForm = function(form, table, nuldlg) {
		this.nldlg = nuldlg;
		this.$form = $('#'+form);
		this.$table = $('#'+table);
		this.$form.append(
		'<div id="'+form+'page_nav" style="text-align: center; padding-top:10px;"><input type="hidden" name="from" id="'+form+'from" value="0" /><label>' + Dict['crpp'] + '</label>'+
		'<input type="text" size="2" maxlength="2" value="10" id="'+form+'limit" name="limit" />'+
		'<input type="submit" value="'+ Dict['prv'] +'" id="'+form+'previous" class="hidden"></input><input type="submit" value="' + Dict['nxt'] + '" id="'+form+'next" class="hidden" ></input></div>');
